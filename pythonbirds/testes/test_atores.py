...
class AtorTestes(TestCase):
    ...
    def assert_colisao_atores_ativos(self, ator, ator2, intervalo=1):
        ...
        Atenção: Esse não é método de teste porque nao se inicia com prefixo
        "test".
        Ele serve apenas para encapsular toda lógica de teste de colisão entre
        dois atores ativos
        """
        # Conferindo status dos dois atores antes da colisão
        frase_status_antes = 'Status deveria ser ativo antes da colisão'
        self.assertEqual(ator.status, ATIVO, frase_status_antes)
        self.assertEqual(ator2.status, ATIVO, frase_status_antes)
        ator.colidir(ator2, intervalo)
        # Conferindo status dos dois atores depois da colisão
        frase_status_depois = 'Status deveria ser destruido depois da colisão'
        self.assertEqual(ator2.status, DESTRUIDO, frase_status_depois)
        self.assertEqual(ator.status, DESTRUIDO, frase_status_depois)
    ...
    def assert_nao_colisao(self, ator, ator2):
        ...
        So seja, eles deve manter seus respectivos status mesmo depois da
        chamada do metodo colidir
        """
